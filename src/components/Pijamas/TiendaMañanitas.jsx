...
const TiendaMañanitas = () => {
    const mañanitas = dataMañanitas ?? [];
            ...
            <div className="max-w-7xl w-full mx-auto flex flex-col items-center text-center gap-8">
                ...
                >
                    <Splide options={splideOptions}>
                        {marcasmañanitas.map((marcamañanitas, index) => (
                            <SplideSlide key={index} className="flex justify-center">
                                {marcamañanitas.slug === 'mañanitas' ? (
                                    <Link to="/Catalogomañanitas" aria-label={`Ir a ${marcamañanitas.nombre}`}>
                                    ...
                                    <Link to={`/catalogo/${marcamañanitas.slug}`} aria-label={`Ir a ${marcamañanitas.nombre}`}>
                                        <div className="bg-white rounded-2xl shadow-lg p-4 w-[200px] h-[200px] flex items-center justify-center hover:scale-105 transition-transform duration-300">
                                            <img
                                                src={marcamañanitas.img?.[0] || "/placeholder.jpg"}
                                                alt={marcamañanitas.nombre}
                        ...
                        <Splide options={splideOptions}>
                            {mañanitas.map((val) => (
                                <SplideSlide key={val.id} className="flex justify-center">
                                    <Link to={`/catalogomañanitas/${val.marcamañanitas.toLowerCase()}`} aria-label={`Ver catálogo de ${val.nombre}`}>
                                        <motion.div
...
export default TiendaMañanitas;